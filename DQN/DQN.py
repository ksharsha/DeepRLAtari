...
def main():  # noqa: D103
    f = open("dqnmemexpreplay_again.txt","w")
    ...
    with tf.Session(config=config) as sess:
        ...
        while iterations>0:
            ...
            if iterations % saveiter ==0:
                numsave = numsave+1
                strs = 'dqnmerweights_again/' + str(numsave) + '.h5'
                primmodel.save(strs)
            ...
            if done:
                ...
                accreward =0 
            mem.append(prevstack, action, reward, done, currstack)
            #print ('Sum is: ',np.sum(np.sum(np.sum(np.abs(currstack - prevstack)))))
            prevstack = np.copy(currstack) 
            ...
            #Updating only once every four iterations
            if iterations % 1 ==0:
