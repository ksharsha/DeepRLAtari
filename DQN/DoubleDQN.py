...
def create_model(window, input_shape, num_actions,
    ...
    model.add(Convolution2D(filters=32,kernel_size=(8, 8),strides=4,activation='relu',input_shape=input_shapes))
    model.add(Convolution2D(filters=64,kernel_size=(4, 4),strides=2,activation='relu'))
...
class policy():
    ...
    def __init__(self):
        epsilon = 1
        decr = 0.0000009 #as described in the paper #doubt1 too high?
...
def main():  # noqa: D103
    f = open("dqndoublememexpreplay.txt","w")
    ...
    initstate = env.reset()
    primmodel = create_model(4,(84,84),env.action_space.n) #primary model
    targetmodel = create_model(4,(84,84),env.action_space.n) #target network
    mem = ReplayMemory() 
    ...
    for ra in range(0,maxepisodes):
        ...
        while iterations>0:
            ...
            if iterations % loadfreq == 0:
                ...
                targetmodel.set_weights(primmodel.get_weights()) #MAke the target network same as the primary network
            if iterations % saveiter ==0:
                numsave = numsave+1
                strs = 'dqndoublemerweights/' + str(numsave) + '.h5'
                primmodel.save(strs)
            ...
            #Passing through the target netowrk now
