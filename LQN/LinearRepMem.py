#!/usr/bin/env python
"""Run Atari Environment with LQN."""
import argparse
import os
import random
import gym
...
def main():  # noqa: D103
    f = open("lqnmemexpreplay_again.txt","w")
    ...
    maxepisodes = 1
    episode = 0
    saveiter = 100000 #represents the frequency with which we save the models
    numsave = 0
    ...
    with tf.Session(config=config) as sess:
        ...
        while iterations>0:
            ...
            #Loads weights to the online network
            ...
            if iterations % saveiter ==0:
                numsave = numsave+1
                strs = 'lqnmerweights_again/' + str(numsave) + '.h5'
                primmodel.save(strs)
            #We will fill the replay memory initially
            ...
            action = pol.ldgreedyepsilon(outs) #Picking the action with a linear fecay greedy epsilon policy
            #print('The action chosen is', action)
            prevstack = np.reshape(prevstack,(84,84,4))
            currstack, reward, done = frame.getcurrstack(env, action, prevstack) #getting the new stack from the action taken
            ...
            if done:
                ...
                f.write(strs)
                accreward =0 
            mem.append(prevstack, action, reward, done, currstack) #Adding to the replay memory
            prevstack = np.copy(currstack) 
            currstack = np.reshape(currstack,(1,imsize[0],imsize[1],4))
            #print('The reward is', reward) 
            accreward = accreward + reward
            sample = random.sample(mem.mem, batch_size) #Sampling from the replay memory to use for the gradient update
            ...
            for m in range(0,batch_size):
                if termsample[m]:
                    ...
                    targets.append(rewardsample[m] + gamma*np.max(qvalues[m]))
            ...
            #Updating the target only once every four iterations
            if iterations % 4 == 0:
