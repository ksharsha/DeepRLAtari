...
def main():  # noqa: D103
    ...
    initstate = env.reset()
    Q1 = create_model(4,(84,84),env.action_space.n)
    Q2 = create_model(4,(84,84),env.action_space.n)
    mem = ReplayMemory() 
    ...
    x2 = tf.placeholder(tf.float32, shape=(None,84,84,4)) 
    out = Q1(x)
    out2 = Q2(x2)
    targetq = tf.reduce_sum(tf.multiply(out, a),reduction_indices = 1)
    targetq2 = tf.reduce_sum(tf.multiply(out2, a),reduction_indices = 1)
    absloss = tf.reduce_mean((y-targetq))
    mseloss = tf.reduce_mean(tf.square(y-targetq))
    absloss2 = tf.reduce_mean((y-targetq2))
    mseloss2 = tf.reduce_mean(tf.square(y-targetq2))
    
    max_grad = 1
    huberloss = tf.where(tf.abs(absloss) < max_grad, 0.5 * tf.square(absloss), max_grad * (
        tf.abs(absloss) - max_grad * 0.5))
    huberloss2 = tf.where(tf.abs(absloss2) < max_grad, 0.5 * tf.square(absloss2), max_grad * (
        tf.abs(absloss2) - max_grad * 0.5))
    trainer = tf.train.AdamOptimizer(lr).minimize(huberloss)
    trainer2 = tf.train.AdamOptimizer(lr).minimize(huberloss2)
    ...
    for ra in range(0,maxepisodes):
        ...
        while iterations>0:
            ...
            #    f.write('Loaded weights to the online network')
            #    Q2.set_weights(Q1.get_weights()) 
            if iterations % saveiter ==0:
                numsave = numsave+1
                strs = 'lindouble/' + str(numsave) + 'Q1.h5'
                Q1.save(strs)
                strs = 'lindouble/' + str(numsave) + 'Q2.h5'
                Q2.save(strs)
            #variable to decide which to train
            model = 1 
            if random.random() < 0.5:
                model = 0
            for j in range(0,minsamples):
            ...
            minsamples = 0
            #prevstack = np.zeros((84,84,4),dtype=np.int) 
            prevstack = np.reshape(prevstack,(1,imsize[0],imsize[1],4))
            if model == 0:
                outs = sess.run(out, feed_dict={x:prevstack})
            else:
                outs = sess.run(out2, feed_dict={x2:prevstack})
            ...
            if model == 0:
                ...
                qvalues = sess.run(out2, feed_dict={x2: currsample})
            else:
                qstate = sess.run(out2, feed_dict={x2: currsample})

                qvalues = sess.run(out, feed_dict={x: currsample})
            targets = []
            sumq = 0
            ...
            if model==0:
                _,lossval = sess.run([trainer, mseloss],feed_dict ={y: targets, a:actionnames, x:prevsample})
            else:
                _,lossval = sess.run([trainer2, mseloss2],feed_dict ={y: targets, a:actionnames, x2:prevsample})
            #print(targets)
